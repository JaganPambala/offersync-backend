...
router.patch("/:id/status", auth, async (req, res) => {
  try {
    const { status, validTill, followUpDate } = req.body;
...
router.delete("/:id", auth, async (req, res) => {
  try {
    // Only allow the HR who owns the offer to delete
    const offer = await OfferService.getOfferById(req.params.id);
    if (!offer) {
      return res
        .status(404)
    ...
    const result = await OfferService.deleteOffer(req.params.id, req.user.id);
