    ...
    try {
        ...
        if (!initiatorHR || !recipientHR || !candidate || !offers) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: initiatorHR, recipientHR, candidate, and offers are required'
            });
        }
...
router.post('/:communicationId/outcome', auth, async (req, res) => {
    try {
        console.log('Record outcome request:', req.params.communicationId, req.body);
        const hrId = req.user.id;
...
router.get('/:communicationId', auth, async (req, res) => {
    try {
        console.log('Get communication details request:', req.params.communicationId);
        const hrId = req.user.id;
