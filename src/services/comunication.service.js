...
class CommunicationService {
  ...
  static async updateStatus(communicationId, newStatus, remarks, hrId) {
    try {
      ...
      if (
        ![
          communication.initiatorHrId.toString(),
          communication.recipientHrId.toString(),
        ].includes(hrId)
      ) {
      ...
      if (!this.isValidStatusTransition(communication.status, newStatus)) {
        throw new Error(
          `Invalid status transition from ${communication.status} to ${newStatus}`
        );
      }
    ...
    try {
      ...
      switch (result) {
        ...
        case "OFFER_ACCEPTED": {
          ...
          await Offer.findByIdAndUpdate(acceptedOfferId, {
            status: "ACCEPTED",
          });
  ...
  static async getCommunicationDetails(communicationId, hrId) {
    try {
      ...
      if (
        ![
          communication.initiatorHrId._id.toString(),
          communication.recipientHrId._id.toString(),
        ].includes(hrId)
      ) {
    ...
    try {
      ...
      return communications.map((comm) => {
        ...
        const responseTimeMinutes =
          comm.metrics.respondedAt && comm.metrics.generatedAt
            ? Math.round(
                (comm.metrics.respondedAt - comm.metrics.generatedAt) /
                  (1000 * 60)
              )
        ...
        const resolutionTimeHours =
          comm.metrics.resolvedAt && comm.metrics.generatedAt
            ? Math.round(
                (comm.metrics.resolvedAt - comm.metrics.generatedAt) /
                  (1000 * 60 * 60)
              )
...
Can we coordinate on this?`;
  ...
  static async handleTimelineAgreement(communication, updates) {
    ...
    const otherOffer = communication.offers.find(
      (o) => o.offerId.toString() !== postponedOfferId
    );
  ...
  static async handleCandidateWithdrawal(communication, updates) {
    ...
    const remainingOffer = communication.offers.find(
      (o) => o.offerId.toString() !== withdrawnOfferId
    );
