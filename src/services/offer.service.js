...
class OfferService {
  ...
  static async listOffers(filters = {}, page = 1, limit = 10) {
    ...
    const candidatesWithOffers = await Promise.all(
      paginatedCandidates.map(async (candidateId) => {
        ...
        // Get all offers for this candidate, and populate hrId with company name and HR details
        ...
        // Format offers according to the new structure, including HR and company info
        const formattedOffers = offers.map((offer) => ({
          ...
          hr: offer.hrId
            ? {
                ...
                whatsapp: offer.hrId.whatsapp
                  ? offer.hrId.whatsapp.phoneNumber
                  : undefined,
                company: offer.hrId.company
                  ? offer.hrId.company.name
                  : undefined,
              }
            : null,
        }));
  ...
  static async getOfferDetails(candidateId) {
    ...
    const competitorCount =
      offers.filter((o) => o.status === "ACTIVE").length - 1;
  ...
  static async updateStatus(offerId, status, timelineData = {}) {
    ...
    if (status === "ACCEPTED") {
      ...
      await Candidate.findByIdAndUpdate(offer.candidateId._id, {
        status: "ACCEPTED",
      ...
      for (const otherOffer of otherOffers) {
        // Update offer status
        await Offer.findByIdAndUpdate(otherOffer._id, {
          $set: {
            status: "ON_HOLD",
            updatedAt: new Date(),
          },
    ...
    const competitionInfo = await this.calculateCompetition(
    ...
    if (timelineData.followUpDate) {
      timelineUpdates["timeline.followUpDate"] = new Date(
    ...
    const updated = await Offer.findByIdAndUpdate(
      offerId,
      {
        $set: Object.fromEntries(
          Object.entries(timelineUpdates).map(([k, v]) => [`timeline.${k}`, v])
        ),
      },
      { new: true }
    );
  ...
  static async updateCompetition(offerId, competitionUpdates) {
    throw new Error(
      "Competition is automatically calculated and cannot be manually updated"
    );
  }
  ...
  static async incrementAnalytics(offerId, field) {
    ...
    if (!allowed.includes(field)) {
      const error = new Error(
        `Invalid analytics field. Allowed: ${allowed.join(", ")}`
      );
      error.code = "VALIDATION_ERROR";
      throw error;
    }
