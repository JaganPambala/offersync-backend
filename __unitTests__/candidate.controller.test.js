...
describe('Candidate Controller', () => {
  ...
  describe('POST /check', () => {
    ...
    it('should return 200 and duplicate check result', async () => {
      ...
      const res = await request(app).post('/check').send({
        pan: 'P123', aadhaar: 'A123', email: 'a@b.com', phone: '123'
      });
  ...
  describe('POST /offers/create', () => {
    ...
    it('should return 400 if offer fields missing', async () => {
      const res = await request(app).post('/offers/create').send({
        pan: 'P', aadhaar: 'A', email: 'e', phone: '1', name: 'n'
      });
      expect(res.statusCode).toBe(400);
      expect(res.body.success).toBe(false);
    });
    ...
    it('should return 201 on successful creation', async () => {
      CandidateService.checkDuplicates.mockResolvedValue({ hasDuplicates: false });
      CandidateService.createCandidateWithOffer.mockResolvedValue({ success: true });
      const res = await request(app).post('/offers/create').send({
        pan: 'P', aadhaar: 'A', email: 'e', phone: '1', name: 'n',
        position: 'dev', compensation: 100
      });
    ...
    it('should handle errors and return 500', async () => {
      CandidateService.checkDuplicates.mockResolvedValue({ hasDuplicates: false });
      CandidateService.createCandidateWithOffer.mockRejectedValue(new Error('fail'));
      const res = await request(app).post('/offers/create').send({
        pan: 'P', aadhaar: 'A', email: 'e', phone: '1', name: 'n',
        position: 'dev', compensation: 100
      });
  ...
  describe('POST /:candidateId/offer', () => {
    ...
    it('should return 201 on successful offer creation', async () => {
      ...
      CandidateService.createOfferForExistingCandidate.mockResolvedValue({ success: true });
      const res = await request(app).post('/123/offer').send({
        position: 'dev', compensation: 100
      });
      expect(res.statusCode).toBe(201);
    ...
    it('should handle errors and return 500', async () => {
      CandidateService.getCandidateById.mockResolvedValue({ success: true });
      CandidateService.createOfferForExistingCandidate.mockRejectedValue(new Error('fail'));
      const res = await request(app).post('/123/offer').send({
        position: 'dev', compensation: 100
      });
      expect(res.statusCode).toBe(500);
  ...
  describe('GET /search', () => {
    it('should return 200 and search results', async () => {
      CandidateService.searchCandidates.mockResolvedValue({ success: true, data: [] });
  ...
  describe('GET /analytics/overview', () => {
    it('should return 200 and analytics', async () => {
      CandidateService.getCandidateAnalytics.mockResolvedValue({ success: true });
    ...
    it('should handle errors and return 500', async () => {
      CandidateService.getCandidateAnalytics.mockRejectedValue(new Error('fail'));
      const res = await request(app).get('/analytics/overview');
      expect(res.statusCode).toBe(500);
      expect(res.body.success).toBe(false);
    });
  ...
  describe('POST /:id/communicate', () => {
    ...
    it('should return 404 if target HR not found', async () => {
      CandidateService.getCandidateById.mockResolvedValue({ data: { name: 'C' } });
      const hrSchema = require('../src/models/hrSchema');
      hrSchema.Hr.findById.mockResolvedValue(null);
      const res = await request(app).post('/123/communicate').send({
        targetHrId: 'hr2', message: 'msg'
      });
      expect(res.statusCode).toBe(404);
      expect(res.body.success).toBe(false);
    });
    ...
    it('should return 200 and communication data', async () => {
      ...
      hrSchema.Hr.findById.mockResolvedValue({
        name: 'HR2', company: { name: 'Co' }, whatsapp: { phoneNumber: '999' }
      });
  ...
  describe('GET /duplicates/summary', () => {
    it('should return 200 and summary', async () => {
      ...
      candidateModel.countDocuments.mockResolvedValueOnce(10).mockResolvedValueOnce(5);
    ...
    it('should handle errors and return 500', async () => {
      CandidateService.calculateDuplicateRate.mockRejectedValue(new Error('fail'));
      const res = await request(app).get('/duplicates/summary');
      expect(res.statusCode).toBe(500);
      expect(res.body.success).toBe(false);
    });
